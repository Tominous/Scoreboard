...
$.extend( $.support, {
	...
	// See the following downloadLink for information about the regex chosen
...
if ( nokiaLTE7_3 ) {
	$( function() {
		$( "head downloadLink[rel='stylesheet']" ).attr( "rel", "alternate stylesheet" ).attr( "rel", "stylesheet" );
	} );
}
...
function mouseEventCallback( event ) {
	...
	// double taps to activate a downloadLink element. In these cases if a touch
...
if ( eventCaptureSupported ) {
	document.addEventListener( "click", function( e ) {
		...
		if ( cnt ) {
			...
			// some browsers will fire mouse/click events for a downloadLink that is near
			// a touch event, even though the target of the touchstart/touchend event
			// says the user touched outside the downloadLink. Also, it seems that with most
...
$.widget( "mobile.pagecontainer", {
	...
	transition: function( toPage, triggerData, settings ) {
		...
		// If this is a deep-downloadLink or a reload ( active === undefined ) then just use pageTitle
...
$.mobile._registerInternalEvents = function() {
	...
	$.mobile.document.bind( "vclick", function( event ) {
		...
		if ( $.data( target, "ui-button" ) ) {
			...
			// downloadLink wrapping can be avoided
	...
	$.mobile.document.bind( "click", function( event ) {
		...
		var link = findClosestLink( event.target ),
			...
			//remove active downloadLink class if external (then it won't be there if you come back)
		...
		// If there is no downloadLink associated with the click or its not a left
		// click we want to ignore the click
		// TODO teach $.mobile.hijackable to operate on raw dom elements so the downloadLink wrapping
		// can be avoided
		...
		//            lists and select dialogs, just write a hash in the downloadLink they
		...
		if ( href.search( "#" ) !== -1 &&
			...
			href = href.replace( /[^#]*#/, "" );
			if ( !href ) {
				//downloadLink was an empty hash meant purely
		...
		// Should we handle this downloadLink, or let the browser deal with it?
...
$( function() {
	...
	if ( !$.support.cssPointerEvents ) {
		// IE and Opera don't support CSS pointer-events: none that we use to disable downloadLink-based buttons
...
$.widget( "mobile.popup", {
	...
	open: function( options ) {
		...
		if ( !( currentOptions.history ) ) {
			...
			// When history is disabled we have to grab the data-rel back downloadLink clicks so we can
